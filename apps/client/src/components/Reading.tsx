...
export const Reading = ({ data}: { data: GetReadings | undefined}) => {
  return (
    <div>
      ...
      <div className="mx-auto mt-10 px-8  max-w-4xl grid grid-cols-1 gap-6 sm:grid-cols-2 font-medium">
        {data?.verses.map((verse) => (
          <p className="font-zilla text-xl" key={verse.idx}>
            <span className="mr-10px inline-block align-top text-xs font-bold text-blue-800 dark:text-blue-300">
              {verse.idx}
            </span>
            {verse.text}
          </p>
